...
- [x] Fix draft/unlisted issues leaking onto the public shop (and site-wide) (2026-07-15, server.js `/api/bootstrap`, src/components/PublicSite.jsx `ShopPage`). Bug: `sanitizePublicData` sent `data.issues` to the public bootstrap with no filter at all, and `ShopPage` rendered every issue with no check of `shop.listedInShop` or `status` — a draft, unlisted issue (e.g. issue-3) still appeared on `/shop`. Fixed at two layers: the bootstrap route now filters to `status === "published"` before the data ever reaches the client (also skips the live-Stripe-price lookup for drafts), and `ShopPage` additionally filters to `shop?.listedInShop` so a published-but-not-for-sale issue still stays off the shop grid. Verified in-browser: `/api/bootstrap` still includes issue-3 (draft) until the Node server is restarted to pick up the server.js change, but the client-side `listedInShop` filter already keeps it off `/shop` today — confirmed only the bundle + "3:10 to Nowhere!" render, matching their `listedInShop: true`/`status: published` data. **Needs a server restart** to fully stop shipping draft issue data to the public bootstrap (not just hiding it in the shop UI).
...
- [ ] Refund/dispute webhook handling (2026-07-16, src/lib/ordersStore.js `updateOrderStatusBySessionId`/`getOrderStatusBySessionId` + server.js `handleChargeRefunded`/`handleChargeDispute`). Not on the original list — scoped after the live webhook only covered `checkout.session.completed`. Webhook now also handles `charge.refunded` (full → `status: "refunded"`, revokes library/download access + drops from revenue/customer/unit stats; partial → `status: "partially_refunded"`, access untouched), `charge.dispute.created` (→ `"disputed"`, access untouched, doesn't override an existing `"refunded"` status), and `charge.dispute.closed` (lost → `"refunded"`; won → reverts `"disputed"` back to `"paid"`). Order→event mapping resolves via `stripe.checkout.sessions.list({ payment_intent })` at event time rather than a new column, so it works retroactively on pre-existing orders. Admin Orders table shows a status badge for non-paid orders (`AdminPage.jsx` `OrderStatusBadge`, styles in `src/index.css` — admin isn't behind the `[data-redesign]` flag). **Only build/syntax-verified so far** (`node --check` + `npm run build` both pass) — still needs: the three new event types added to the live Stripe webhook subscription, and an actual Stripe test-mode `stripe trigger charge.refunded`/`charge.dispute.created`/`charge.dispute.closed` pass to confirm status transitions and access revocation end-to-end.
- [ ] Cart trust-badge row — official Apple Pay + Google Pay marks + generic "Cards" glyph (2026-07-16, src/components/Cart.jsx `CartSummary` + src/assets/payment-badges/ + src/redesign.css `.cart-summary__paymentIcons`/`.cart-summary__cardsLabel`). Not on the original list — a cosmetic conversion-trust addition, separate from actually enabling the wallets. Apple's and Google's marks are the actual official brand assets (Apple's from developer.apple.com/apple-pay/marketing/, Google's from developers.google.com Google Pay brand guidelines), copied into the repo byte-for-byte (verified via `diff` against the downloaded originals, not hand-transcribed) and rendered as `<img>` at native color/no opacity per each brand's "don't alter the mark" guideline. The "Cards" glyph stays a hand-drawn generic icon (no card-network logos sourced) since it isn't a trademarked mark. Redesign-scoped only (`[data-redesign="1"]`), legacy cart untouched. **Build-verified** (`npm run build` passes) but **not yet verified in-browser** — no dev server was running this session to screenshot against; needs a visual check (renders above the checkout button, doesn't clip on mobile, doesn't compete visually with the ember CTA).
- [ ] Limited physical stock (2026-07-16, server.js + src/components/PublicSite.jsx `ShopFormatToggle` + src/components/Cart.jsx + src/components/AdminPage.jsx `IssueEditor`). Not on the original list — user has a limited print run to sell without overselling. New optional `issue.shop.physicalStock` (integer, absent = unlimited), editable in the admin Physical card. Enforced at four points: storefront (`ShopFormatToggle` shows "Physical — Sold out" disabled at 0, "Only N left" at ≤5 remaining), cart (`Cart.jsx` clamps `addItem`/`setQuantity` to remaining stock and the existing stale-cart cleanup effect now also drops/clamps physical lines against current stock), `/api/checkout` (rejects a request exceeding remaining stock), and the fulfillment webhook (decrements `physicalStock` after `ordersStore.insertOrder`, best-effort/non-fatal like the confirmation email send next to it). **Prerequisite fix bundled in:** the webhook previously collapsed a quantity>1 Stripe line item into a single `purchasedItems` row (silently undercounting order history/units-sold stats); it now expands into one row per unit, mirroring the existing bundle-expansion pattern. `repository.decrementPhysicalStock` (both `PgRepository`/`FileRepository`) uses the same non-locked read/mutate/write shape as every other write in this repository — a documented, accepted tradeoff for a small print run, not real inventory-grade locking. **Only build/syntax-verified so far** (`node --check` + `npm run build` both pass) — still needs an in-browser + Stripe test-mode pass: set stock to 1 on an issue, confirm the "Only 1 left" copy and cart clamp, complete a test checkout, confirm stock drops to 0 and the format flips to disabled "Sold out" on reload.
- [ ] Admin sale notification email (2026-07-16, src/lib/resendEmail.js `buildAdminSaleNotificationEmail` + server.js webhook handler). Not on the original list — user wants to know immediately when a sale happens and when a physical order needs fulfillment. Rides the existing Resend infra (same `sendResendEmail`, same best-effort/non-fatal pattern as the customer confirmation email beside it) rather than a new service. Fires on every `checkout.session.completed` to `ADMIN_NOTIFICATION_EMAIL` (env-overridable, defaults to `renownedcomicbook@gmail.com`), subject/copy differing when any purchased item is physical ("New physical order to fulfill" + shipping address vs. "New sale"). Body includes an issue+format+quantity summary (`summarizePurchasedItems`, collapsing the one-row-per-unit `purchasedItems` back into readable lines like "2x Issue 1 (physical)"), total paid, and customer email. **Only build/syntax-verified so far** (`node --check` + `npm run build` both pass) — still needs a real Resend send verified (a test checkout with `RESEND_API_KEY`/`RESEND_FROM_EMAIL` configured, confirming the email actually arrives with correct subject/content for both a digital-only and a physical order).

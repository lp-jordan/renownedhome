...
- [ ] Cart trust-badge row — "Apple Pay · Google Pay · Cards" (2026-07-16, src/components/Cart.jsx `CartSummary` + src/redesign.css `.cart-summary__paymentIcons`). Not on the original list — a cosmetic conversion-trust addition, separate from actually enabling the wallets. Deliberately uses a generic card glyph + text label rather than the official Apple Pay/Google Pay marks (avoids reproducing trademarked logo art without their brand assets). Redesign-scoped only (`[data-redesign="1"]`), legacy cart untouched. **Not yet verified in-browser** — no dev server was running this session to screenshot against; needs a visual check (renders above the checkout button, doesn't clip on mobile, doesn't compete visually with the ember CTA).
